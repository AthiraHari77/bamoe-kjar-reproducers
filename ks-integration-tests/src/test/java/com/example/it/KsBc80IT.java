...
/**
 * KsBc80IT — Integration tests for the kjar-bc80-to-ks81 scenario.
 *
 * KJAR: example-kjar 1.0.0, authored in Business Central 8.0
 *       (KIE 7.67.2.Final-redhat-00034), includes scesim test files.
 * Target: KIE Server 8.1
 *
 * Validates that a KJAR exported from BC 8.0 (with DMN, DRL, BPMN, PMML
 * and scesim test scenarios) can be deployed and executed on KIE Server 8.1.
 */
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
@DisplayName("kjar-bc80 (authored in BC 8.0) → KIE Server 8.1")
public class KsBc80IT extends KsTestBase {

    static final String CONTAINER_ID  = "example-kjar-bc80_1.0.0";
    ...
    static void deployKjar() throws Exception {
        ...
        Path jarPath = Paths.get(scenarioRoot,
            "kjar-bc80-to-ks81", "kjar", "target", "example-kjar-1.0.0.jar");
        Path pomPath = Paths.get(scenarioRoot,
            "kjar-bc80-to-ks81", "kjar", "pom.xml");

        assertTrue(jarPath.toFile().exists(),
            "KJAR not found at " + jarPath + " — run 'mvn clean install' in kjar-bc80-to-ks81/kjar first");
